...
def _serialize(obj):
    """Serialize any Python object into printable / JSON-friendly dict / list / primitive types"""
    # 1. 基础类型
...
# Environment operation status枚举
...
async def publish_control_signal(
    ...
    **kwargs,
    ...
    """Publish control signal to Redis"""
    ...
    try:
        ...
        logger.info(f"Publishing control signal: {action} to {channel}")
        return True
    except Exception as e:
        logger.error(f"Failed to publish control signal: {e}")
        return False


async def listen_for_control_signals(user_id: str, chat_id: str, agent):
    """Background task listening for Redis control signals"""
    if not state_manager:
        return
    ...
    try:
        ...
        logger.info(f"Start listening for control signal on: {channel}")
        ...
        async for message in pubsub.listen():
            if message["type"] == "message":
                try:
                    ...
                    logger.info(f"Received control signal: {action} from {channel}")
                    ...
                    logger.error(f"Error handling control signal: {e}")

    except Exception as e:
        logger.error(f"Failed to listen for control signal: {e}")
    finally:
        try:
            await pubsub.unsubscribe(channel)
            await pubsub.close()
        except Exception as e:
            logger.error(f"Failed to close pub/sub connection: {e}")


async def check_stop_signal_from_redis(user_id: str, chat_id: str) -> bool:
    """Check stop signal in Redis"""
    try:
    ...
    **kwargs,
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"Redis operation timed out, retry {attempt + 1}/{max_retries}")
            if attempt < max_retries - 1:
                await asyncio.sleep(1.0 * (attempt + 1))  # 递增延迟
            else:
                logger.error("Redis operation ultimately timed out")
                return None
        except Exception as e:
            logger.warning(
                f"Redis operation failed, retry {attempt + 1}/{max_retries}: {e}",
            )
            if attempt < max_retries - 1:
                await asyncio.sleep(1.0 * (attempt + 1))
            else:
                logger.error(f"Redis operation ultimately failed: {e}")
                return None
    return None
...
class ComputerUseRequest(AgentRequest):
    """Extended agent request including ComputerUse-specific settings"""
...
async def get_equipment(
    ...
    async with chat_state.lock:
        ...
        if chat_state.equipment is not None:
            logger.info(f"Device already exists: {chat_id}-{sandbox_type}")
            if sandbox_type == "pc_wuyin":
                ...
                try:
                    ...
                    if (
                        ...
                        logger.warning(
                            f"PC device in abnormal state, reinitializing: {instance_m.desktop_id}",
                        )
                        # 清理旧设备
                    ...
                    # 重新获取auth_code确保有效性
                    logger.info(
                        f"Refreshing auth_code to ensure validity, desktop_id: "
                    ...
                    if new_auth_code:
                        ...
                        logger.info(
                            f"auth_code updated: {instance_m.desktop_id}",
                        )
                    else:
                        logger.warning(
                            f"Failed to obtain new auth_code; using previous auth_code: "
                            f"{instance_m.desktop_id}",
                        )

                except Exception as e:
                    logger.error(f"Error validating PC device: {e}; reinitializing")
                    # 清理旧设备
                    chat_state.equipment = None
                    # 递归调用，重新初始化设备
                    return await get_equipment(user_id, chat_id, config)
                ...
                logger.info(
                    f"Device already exists; returning device info - desktop_id: {instance_m.desktop_id}",
                )
                return {
                    "task_id": task_id,
                ...
                try:
                    ...
                    if (
                        ...
                        logger.warning(
                            f"Phone device in abnormal state, reinitializing: {instance_m.instance_id}",
                        )
                        # 清理旧设备
                        chat_state.equipment = None
                        # 递归调用，重新初始化设备
                    ...
                    logger.error(f"Error validating phone device: {e}; reinitializing")
                    # 清理旧设备
                    chat_state.equipment = None
                    # 递归调用，重新初始化设备
                    return await get_equipment(user_id, chat_id, config)
                ...
                logger.info(
                    f"Device already exists; returning device info - instance_id: {instance_m.instance_id}",
                )
                return {
                    "task_id": task_id,
                    "equipment_web_url": chat_state.equipment_web_url,
            ...
            if sandbox_type == "pc_wuyin":
                # 从资源分配器获取PC实例
                logger.info(
                    f"Starting user_id: {user_id}, chat_id: {chat_id}",
                )
                desktop_id, status = (
                ...
                logger.info(
                    f"Starting desktop_id: {desktop_id}, status: {status}",
                )
                # 处理资源排队情况
                ...
                try:
                    ...
                    # 初始化失败时释放已分配的资源
                    logger.error(
                        f"CloudComputer init failed: {e}; releasing resource {desktop_id}",
                    )
                ...
                logger.info(
                    f"Starting equipment: {equipment}-{equipment.instance_manager}"
            ...
            # 初始化手机设备
                ...
                logger.info(
                    f"Starting instance_id: {instance_id}, status: {status}",
                )
                # 处理资源排队情况
                if status == AllocationStatus.WAIT_TIMEOUT:
                ...
                try:
                    ...
                    # 🚨 初始化失败时释放已分配的资源
                    logger.error(
                        f"CloudPhone init failed: {e}; releasing resource {instance_id}",
                    )
                ...
                logger.info(f"Starting instance_id: {instance_id}")
...
async def _handle_resume_stream(
    ...
    """Handling resume-from-disconnect, returning historical data"""
    logger.info(
        f"Resume-from-disconnect for user: {user_id}, chat: {chat_id}, from sequence: {from_sequence}",
    )
    ...
    async def resume_stream():
        try:
            ...
            if chat_state.get("is_running") and chat_state.get("agent"):
                logger.info("Detected running task, continuing to stream new data")
            ...
            logger.error(f"Error handling resume-from-disconnect: {e}")
...
async def _handle_new_stream(
    ...
    """Handling new streaming task"""
    logger.info(f"Starting new task, user: {user_id}, chat: {chat_id}")

    async def agent_stream():
        """Agent streaming response generator with sequence storage"""
        task_id = None
        try:
            ...
            equipment_obj = chat_state.get("equipment")
            if sandbox_type == "e2b_desktop":
                # 对于E2B设备，从状态管理器获取实际的沙盒实例
                equipment_obj = state_manager.get_e2b_sandbox(
                    user_id,
                    chat_id,
                )
            ...
            logger.info(f"Agent execution started, user: {user_id}, chat: {chat_id}")
            # 执行Agent任务并处理流式输出
            async_iterator = None
            ...
            try:
                ...
                async for result in async_iterator:
                    ...
                    if current_time - last_heartbeat >= heartbeat_interval:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to store heartbeat to Redis: {heartbeat_redis_error}",
                            )
                    ...
                    try:
                        ...
                        logger.error(f"Error processing output: {serialize_error}")
                        # 存储错误信息
                        error_data = {
                            "error": f"Error serializing output: {str(serialize_error)}",
                        ...
                        if sequence_number is not None:
                            ...
                            error_output = {
                                ...
                                "type": "error",
                                "data": error_data,
                                "storage_warning": "Failed to store error information to Redis",
                            }
                            _data = json.dumps(
                                error_output,
                                ensure_ascii=False,
                ...
                logger.error(f"Error during agent execution: {iteration_error}")
                # 存储执行错误
                error_data = {
                    "error": f"Error during agent execution: {str(iteration_error)}",
                ...
                if sequence_number is not None:
                    ...
                    error_output = {
                        ...
                        "type": "error",
                        "data": error_data,
                        "storage_warning": "Execution error: failed to store error information to Redis",
                    }
                    _d = json.dumps(error_output, ensure_ascii=False)
                    yield f"data: {_d}\n\n"
                ...
                if async_iterator and hasattr(async_iterator, "aclose"):
                    try:
                        ...
                        print(f"Error closing async iterator: {close_error}")

            print(f"Agent execution finished, user: {user_id}, chat: {chat_id}")
            ...
            try:
                ...
                if sequence_number is not None:
                    ...
                    error_output = {
                        ...
                        "type": "error",
                        "data": error_data,
                        "storage_warning": "Global error: failed to store error information to Redis",
                    }
                    _d = json.dumps(error_output, ensure_ascii=False)
                    yield f"data: {_d}\n\n"
            except Exception as storage_error:
                print(f"Failed to store error information: {storage_error}")
                # 最后的错误输出，不存储到Redis
                final_error = {
                    "sequence_number": None,
                    "object": "error",
            ...
            try:
                ...
                print(f"Error while cleaning up state: {cleanup_error}")
...
async def startup_event():
    ...
    try:
        ...
        logger.info(f"Machine {MACHINE_ID} registered in Redis")
    except Exception as e:
        logger.error(f"Failed to register machine info: {e}")
...
async def init_task(request: InitRequest, user_id: str = ""):
    """Triggering asynchronous environment initialization"""
    if not user_id:
        if request.user_id:
    ...
    logger.info(
        f"Received task request, user: {user_id}, chat: {chat_id} "
...
async def switch_environment(request: InitRequest):
    """Triggering asynchronous environment switch"""
...
async def get_operation_status(
    ...
    """查询Environment operation status（用于轮询）"""
    if not user_id or not chat_id:
...
async def run_task_stream(request: ComputerUseRequest, user_id: str = ""):
    ...
    logger.info(
        f"Received task request, user: {user_id}, chat: {chat_id}, "
        f"Sequence: {sequence_number} , request: "
...
async def stop_task(user_id: str, chat_id: str):
    ...
    try:
        ...
        if agent_machine:
            logger.info(f"Stop signal sent to machine {agent_machine}")
        else:
            logger.info("Agent machine info not found; signal broadcasted")
    except Exception as e:
        logger.error(f"Failed to obtain agent machine info: {e}")
    ...
    try:
        ...
        if local_agent and hasattr(local_agent, "should_stop"):
            ...
            logger.info("Local agent stop flag set")

        # 如果找到本地Agent，也将其从字典中移除
        if local_agent:
            del app.state.running_agents[composite_key]
            logger.info("Local agent reference cleared")
    except Exception as e:
        logger.error(f"Failed to set local agent stop flag: {e}")
...
async def interrupt_wait(user_id: str, chat_id: str):
    ...
    # 记录运行Agent的机器信息
    if agent_machine:
        logger.info(f"Interrupt-wait signal sent to machine {agent_machine}")
    else:
        logger.info("Agent machine info not found; signal broadcasted")
    ...
    try:
        ...
        if local_agent and hasattr(local_agent, "interrupt_wait"):
            ...
            logger.info("Local agent interrupt-wait invoked")
        elif local_agent is None:
            logger.info("Local agent instance not found")
        else:
            logger.warning("Local agent does not support interrupt_wait")
    except Exception as e:
        logger.error(f"Failed to invoke local agent interrupt_wait: {e}")
