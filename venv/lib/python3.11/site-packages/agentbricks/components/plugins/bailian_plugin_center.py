...
from agentbricks.components.internal.clients.http_client import HttpClient
from agentbricks.errors.service_errors import ToolSchemaCenterConnectionError
...
from agentbricks.utils.utils import json_loads
...
class BailianSearchPluginSchema(
    ...
    @trace(
        trace_type=TraceType.PLUGIN,
        trace_name="bailian_search_plugin_schema",
    )
    async def _arun(
        self,
        ...
        **kwargs: Any,
        ...
        """Search plugin schema from Bailian Plugin center.

        This method searches for plugin schemas based on the provided plugin
        names and returns the schema information for those plugins.

        Args:
            args: SearchPluginSchemaInput containing plugin names and
            request ID.
            **kwargs: Additional keyword arguments.

        Returns:
            SearchPluginSchemaOutput containing the plugin schema information
            as a dictionary, or empty dict if an error occurs.
        """
        try:
    ...
    def get_plugin_schema_request(
        ...
        """Get plugin schema information for the requested plugins.

        This method coordinates the retrieval of plugin schemas from various
        sources and consolidates them into a single response.

        Args:
            search_input: SearchPluginSchemaInput containing plugin names
                and request information.

        Returns:
            Dictionary containing plugin schema information for all
            requested plugins.
        """
    ...
    def plugin_schema_request(
        ...
        """Make HTTP request to retrieve plugin schemas from DashScope.

        This method handles the actual HTTP communication with the DashScope
        plugin schema service to retrieve schema information for the
        requested plugins.
        ...
        Returns:
            Dictionary containing the plugin schema information retrieved
            from the DashScope service.

        Raises:
            ToolSchemaCenterConnectionError: If the connection to the schema
                center fails or returns an error status.
        """
        tool_schema_headers = {
...
class BailianPlugin(Component[PluginInput, PluginOutput]):
    ...
    @trace(trace_type=TraceType.PLUGIN, trace_name="bailian_plugin")
...
class BailianPluginInvoke(Component[PluginInvokeInput, PluginInvokeOutput]):
    ...
    @trace(trace_type=TraceType.PLUGIN, trace_name="bailian_plugin_invoke")
    async def _arun(
        self,
