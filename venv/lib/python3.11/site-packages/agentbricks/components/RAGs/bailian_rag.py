...
from agentbricks.constants import DASHSCOPE_HTTP_BASE_URL, DASHSCOPE_API_KEY
...
from agentbricks.schemas.oai_llm import UserMessage
...
class RagInput(BaseModel):
    ...
    workspace_id: Optional[str] = Field(
        "",
        description="user workspace id could be found at bailian",
    )
...
class BailianRag(Component[RagInput, RagOutput]):
    ...
    description: str = (
        "Bailian_Rag可召回用户在百炼上的数据库中存储的信息，当用户要求搜索图片"
        "或者指定库里信息的时候，请优先调用该工具在用户自己库里的查找一下是否有相关信息。"
    )
    name: str = "bailian_RAG"

    @trace(trace_type=TraceType.RAG, trace_name="bailian_rag")
    async def _arun(self, args: RagInput, **kwargs: Any) -> RagOutput:
        """RAG component to retrieve and augment user data on Bailian.

        This method performs RAG by querying the user's knowledge base on
        Bailian platform and updating the system prompt with the retrieved
        information.
        ...
        try:
            ...
            payload, headers = await BailianRag.generate_rag_request(
                args,
                **kwargs,
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response_json.get("data"):
                        ...
                        output_messages = BailianRag.update_system_prompt(
                            args,
                            result,
                        )
                        if is_function_call:
    ...
    async def generate_rag_request(
        ...
        **kwargs: Any,
        ...
        headers needed for the Bailian RAG service API call.
        ...
        async def _build_body(_rag_input: RagInput, base_url: str) -> dict:
            ...
            if _rag_options.index_names:
                tasks = [
                    BailianRag.get_pipeline_id(api_key, base_url, index_name)
            ...
            data = await BailianRag.get_body(_rag_input, pipeline_id_list)
    ...
    async def get_pipeline_id(
        ...
        try:
            ...
            raise Exception(f"get pipeline id exceptionally: {str(e)}")
