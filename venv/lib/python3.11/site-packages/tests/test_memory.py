...
from agentbricks.components.memory.local_memory import (
    LocalMemory,
    MemoryInput,
    MemoryOperation,
    SimpleChatStore,
...
def test_delete_message(chat_store):
    key = "test_key"
    messages = [
        UserMessage(content="Hello world", name=None),
        UserMessage(content="Hello again", name=None),
    ]
    chat_store.set_messages(key, messages)
...
async def test_put_and_get_all(memory):
    message = UserMessage(content="Hello, world", name=None)
    await memory.add(
        MemoryInput(
