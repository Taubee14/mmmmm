...
from cua_utils import init_sandbox, cleanup_sandbox
import os
...
class RedisStateManager:
    ...
    def __init__(
        ...
        # 本地缓存E2B沙盒实例，避免序列化问题
        self._e2b_sandboxes: Dict[str, Any] = {}
    ...
    def set_e2b_sandbox(self, user_id: str, chat_id: str, sandbox: Any):
        """缓存E2B沙盒实例"""
        composite = self._composite_key(user_id, chat_id)
        old_sandbox = self._e2b_sandboxes.get(composite)
        if old_sandbox and old_sandbox is not sandbox:
            try:
                cleanup_sandbox(old_sandbox)
            except Exception as e:
                logger.warning(f"清理旧E2B沙盒失败: {e}")
        self._e2b_sandboxes[composite] = sandbox

    def get_e2b_sandbox(self, user_id: str, chat_id: str) -> Optional[Any]:
        """获取已缓存的E2B沙盒实例"""
        composite = self._composite_key(user_id, chat_id)
        return self._e2b_sandboxes.get(composite)

    def remove_e2b_sandbox(self, user_id: str, chat_id: str):
        """移除并清理E2B沙盒实例"""
        composite = self._composite_key(user_id, chat_id)
        sandbox = self._e2b_sandboxes.pop(composite, None)
        if sandbox:
            try:
                cleanup_sandbox(sandbox)
            except Exception as e:
                logger.warning(f"清理E2B沙盒失败: {e}")
    ...
    async def cleanup_chat_data(self, user_id: str, chat_id: str):
        ...
        # 先移除本地缓存的E2B沙盒，避免资源泄露
        self.remove_e2b_sandbox(user_id, chat_id)
    ...
    async def _retry_environment_init(
        ...
        try:
            ...
            await self.update_environment_operation(
                ...
                {
                    "status": "completed",
                    "message": "Environment initialization complete (queue retry succeeded)",
                    "progress": 100,
                    "end_time": time.time(),
                    "result": result,
                },
    ...
    async def _init_equipment_async(
        ...
        # 默认e2b桌面初始化
            ...
            self.set_e2b_sandbox(user_id, chat_id, equipment)
            ...
            await self.update_chat_state(
                ...
                chat_id,
                {
                    ...
                    "task_id": task_id,
                    "sandbox_type": sandbox_type,
                    "equipment_storage_status": "e2b_desktop",
                },
            )
