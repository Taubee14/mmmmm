...
class ImageGenInput(BaseModel):
    ...
    watermark: bool = Field(
        default=False,
        description="是否添加水印，水印位于图片右下角，文案为“AI生成”",
    )
...
class ImageEdit(Component[ImageGenInput, ImageGenOutput]):
    ...
    name: str = "bailian_image_edit"
    description: str = (
        "AI图像编辑（图生图）服务，输入原图URL、编辑功能、文本描述和分辨率，"
        "返回编辑后的图片URL。"
    )

    @trace(trace_type=TraceType.AIGC, trace_name="image_edit")
    async def arun(self, args: ImageGenInput, **kwargs: Any) -> ImageGenOutput:
        """Bailian image editing from base image and text prompts
        ...
        if args.mask_image_url:
            call_args["mask_image_url"] = args.mask_image_url

        # 🔄 使用DashScope异步任务API实现真正的并发
        # 1. 提交异步任务
        task_response = ImageSynthesis.async_call(**call_args)
